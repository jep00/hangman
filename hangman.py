...
class Hangman:
    ...
    def __init__(self, answer: str, difficulty: str = 'easy') -> None:
        ...
        self.n_guesses_remaining = int() # Will be used to store the number of guesses remaining
        self.n_guesses_start = int() # Will be used to store the number of guesses allowed at the beginning of the game (used to draw the hangman image)
        ...
        os.system('clear') # Clears the screen so the inputted answer is no longer visible
    ...
    def validate_word(self, word:str) -> bool:
        ...
        if re.search(pattern, word):
            raise ValueError('invalid character entered. no numbers or special characters allowed.')

        if word not in WORDS:
            overwrite = str(input(f'{word} is not an accepted word. Would you like to continue? Y/N: '))
    ...
    def set_difficulty(self, difficulty: str) -> None:
        ...
        self.n_guesses_start = self.n_guesses_remaining # Number of guesses allowed at the start
    ...
    def set_guesses(self) -> int:
        ...
        if len(self.answer) < 5 and self.difficulty=='easy':
            return 9
        if (len(self.answer) >= 5 and self.difficulty=='easy') or (len(self.answer) < 5 and self.difficulty=='hard'):
            return 7
        else:
            return 5
    ...
    def print_answers(self) -> str:
        ...
        print('Your correct letters so far: '+(' '.join(
            [w if w in self.correct_guesses else "_" for w in self.answer]
        )))
